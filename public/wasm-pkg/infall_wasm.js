 ...
 * @returns {number}
 */
export function wasm_ergosphere_radius(mass, spin, theta) {
    const ret = wasm.wasm_ergosphere_radius(mass, spin, theta);
    return ret;
}
 ...
 * @returns {number}
 */
export function wasm_event_horizon(mass, spin) {
    const ret = wasm.wasm_event_horizon(mass, spin);
    return ret;
}
 ...
 * @param {number} mass
 * @param {number} spin
 * @param {number} initial_r
 * @returns {any}
 */
export function wasm_init(mass, spin, initial_r) {
    const ret = wasm.wasm_init(mass, spin, initial_r);
    if (ret[2]) {
 ...
 * @returns {number}
 */
export function wasm_isco_radius(mass, spin, prograde) {
    const ret = wasm.wasm_isco_radius(mass, spin, prograde);
    return ret;
}
 ...
 * @returns {any}
 */
export function wasm_step(state_js, dtau) {
    const ret = wasm.wasm_step(state_js, dtau);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('infall_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
